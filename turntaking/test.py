...
from os.path import join, dirname, isfile, basename, exists
import hydra 
import torch
import json
import pandas as pd
...
from turntaking.utils import everything_deterministic, set_seed, to_device, set_debug_mode, write_json
...
class Test():
    def __init__(self, conf, dm, model_path):
        ...
        self.model.load_state_dict(torch.load(model_path, map_location=conf["train"]["device"]))
        ...
        if exists(join(dirname(model_path), "thresholds.json")):
            with open(join(dirname(model_path), "thresholds.json")) as f:
                ...
                print("Loading thresholds: ", join(dirname(model_path), "thresholds.json"))
                self.thresholds = {
    ...
    def _find_threshold(self):
        ...
        print("#" * 60)
        thresholds, prediction, curves = roc_to_threshold(self.conf, self.model, self.dm)
        print("-" * 60)
        print("### Thresholds ###")
        ...
        ### Write ###
        th = {k: v.item() if isinstance(v, torch.Tensor) else v for k, v in thresholds.items()}
    ...
    def _extract_data(self, data, keys, start_idx, end_idx, waveform_scale_factor=640):
        ...
        for key in keys:
            if key in {"waveform", "waveform_expert", "waveform_novice"}:
                extracted_data[key] = data[key][:, start_idx*waveform_scale_factor:end_idx*waveform_scale_factor]
            else:
    ...
    def img(self, time_duration = 30):
        ...
        if self.conf["data"]["multimodal"]:
            data_keys.extend(["gaze_expert", "au_expert", "head_expert", "pose_expert", "gaze_novice", "au_novice", "head_novice", "pose_novice"])

        prob_keys = ["p", "bc_prediction"]
        event_keys = ["shift", "predict_bc_pos"]

        for ii in tqdm(range(int(data["vad"].size(-2)/(time_duration*self.conf["model"]["encoder"]["frame_hz"])))):
            start_time, end_time = self._extract_time_slice(self.conf, data, ii, time_duration)
            
            # Prepare data for the current time frame
            d = self._extract_data(data, data_keys, start_time, end_time, waveform_scale_factor=int(self.conf["model"]["encoder"]["sample_rate"]/self.conf["model"]["encoder"]["frame_hz"]))

            # Prepare probabilities for the current time frame
            p = self._extract_data(self.turn_taking_probs, prob_keys, start_time, end_time)
